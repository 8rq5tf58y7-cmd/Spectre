...
from rtx_converter import (
    ...
    _build_spectrum_run_ids,
    _build_line_scan_run_ids,
    _find_full_deconv_spectrum_indices,
    _decode_binary_float64_series,
    LineScanExporter,
    ElementMapMSAExporter,
    EMSAExporter,
    MetadataExporter,
)
...
class TestFullDeconvSelection(unittest.TestCase):
    def test_find_full_deconv_spectrum_indices(self):
        spectra = [
            {'name': 'deconv', 'counts': [1, 2, 3], 'meta': {}},
            {'name': 'deconv', 'counts': [1, 2, 3], 'meta': {}},
            {'name': 'Spot 1 Deconvoluted', 'counts': [1, 2, 3], 'meta': {}},
            {'name': 'Scan', 'counts': [1, 2, 3], 'meta': {}},
        ]
        # Index 1 is mapped to per-element deconv and should be excluded.
        got = _find_full_deconv_spectrum_indices(spectra, {1})
        self.assertEqual(got, [0])


class TestRunIdAssignment(unittest.TestCase):
    def test_build_spectrum_run_ids_uses_datetime(self):
        spectra = [
            {'name': 'Spot 1', 'counts': [1, 2], 'meta': {'Date': '11.3.2026', 'Time': '10:49:24'}},
            {'name': 'Spot 1', 'counts': [3, 4], 'meta': {'Date': '11.3.2026', 'Time': '10:49:24'}},
            {'name': 'Spot 1', 'counts': [5, 6], 'meta': {'Date': '11.3.2026', 'Time': '11:03:38'}},
        ]
        ids = _build_spectrum_run_ids(spectra, {})
        self.assertEqual(ids, [1, 1, 2])

    def test_build_line_scan_run_ids(self):
        blocks = [
            {'date': '11.3.2026', 'time': '10:49:24', 'scan_length': 1900.0, 'count': 9},
            {'date': '11.3.2026', 'time': '11:3:38', 'scan_length': 1900.0, 'count': 15},
        ]
        ids = _build_line_scan_run_ids(blocks)
...
class TestLabelDeduplication(unittest.TestCase):
    ...
    def test_sum_label_simple_duplicate_suffix(self):
        seen = set()
        type_counts = {}
        labels = [
            _deduplicate_label('sum', seen, type_counts),
            _deduplicate_label('sum', seen, type_counts),
            _deduplicate_label('sum', seen, type_counts),
        ]
        self.assertEqual(labels, ['sum', 'sum_2', 'sum_3'])
...
class TestElementMapMSAExporter(unittest.TestCase):
    def test_export_flattened_msa(self):
        parser = _FakeParser([{'name': 'Spot 1', 'counts': [1, 2, 3], 'meta': {}}])
        exporter = ElementMapMSAExporter(parser)
        map_item = {
            'symbol': 'mn',
            'description': 'Mn-L',
            'width': 2,
        ...
        with tempfile.NamedTemporaryFile(mode='r', suffix='.msa', delete=False) as f:
            path = f.name
        try:
            exporter.export_flattened_msa(map_item, path, title='deconv_map_mn_2d_map_flattened_for_hyperspy')
            with open(path, encoding='latin-1') as f:
                txt = f.read()
            self.assertIn('#SIGNALTYPE  : EDS_ELEMENT_MAP_2D_FLATTENED', txt)
            self.assertIn('#COMMENT     : MapWidth=2', txt)
            self.assertIn('#COMMENT     : MapHeight=2', txt)
            self.assertIn('#NPOINTS     : 4', txt)
        finally:
            os.unlink(path)
