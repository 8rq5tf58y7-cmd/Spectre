...
def _parse_float(text: str) -> Optional[float]:
    """Parse a float-like text value; returns None if not parseable."""
    if text is None:
        return None
    t = text.strip()
    if not t:
...
def _decode_binary_float64_series(encoded_data: str, expected_size: Optional[int] = None) -> List[float]:
    """Decode Bruker TRTBinaryData payload (base64 little-endian float64)."""
    raw = base64.b64decode((encoded_data or '').strip())
    if expected_size is not None and expected_size > 0 and len(raw) != expected_size:
        raise ValueError(f"Binary payload size mismatch: expected {expected_size}, got {len(raw)}")
    if len(raw) % 8 != 0:
        raise ValueError(f"Binary payload is not float64-aligned: {len(raw)} bytes")
    count = len(raw) // 8
    vals = struct.unpack('<' + 'd' * count, raw)
    return [float(v) for v in vals]


def _decode_utf16_prefixed_label(text: str) -> str:
    """Decode Bruker UTF16_ base64 label if present."""
    t = (text or '').strip()
    if not t.startswith('UTF16_'):
        return t
    try:
        payload = base64.b64decode(t[len('UTF16_'):])
        return payload.decode('utf-16-le', errors='ignore').replace('\x00', '').strip()
    except Exception:
        return t


def _build_sample_token(metadata: Dict[str, str], fallback: str) -> str:
    """Build a stable sample token for filenames."""
    for key in ('project_comment', 'project_creator', 'acquisition_date'):
        v = (metadata.get(key) or '').strip()
        if v:
            tok = _sanitize_label(v)
            if tok:
                return tok
    tok = _sanitize_label(fallback)
    return tok or 'sample'


def _normalize_time_token(raw_time: str) -> str:
    """Normalize time text to HH:MM:SS where possible."""
    t = (raw_time or '').strip()
    if not t:
        return ''
...
def _build_spectrum_run_ids(spectra: List[Dict], metadata: Dict[str, str]) -> List[int]:
    """Assign run IDs for spectra using Date/Time and count-based fallback.

    Primary discriminator: spectrum header Date+Time (or project fallback).
    Fallback discriminator when datetime is missing: coarse count signature.
    """
    run_ids: List[int] = []
    key_to_run: Dict[str, int] = {}
    next_run = 1

    project_date = metadata.get('acquisition_date', metadata.get('project_date', ''))
    project_time = metadata.get('acquisition_time', metadata.get('project_time', ''))

    for sp in spectra:
        meta = sp.get('meta', {})
        date = (meta.get('Date') or project_date or '').strip()
        time = _normalize_time_token(meta.get('Time', project_time))
        ...
        if date or time:
            ...
            chans = len(counts)
            # coarse bin to avoid over-fragmenting by noise
            total_bin = (total // 1000) * 1000 if total >= 1000 else total
            key = f'cnt:{chans}:{total_bin}'

        if key not in key_to_run:
            key_to_run[key] = next_run
            next_run += 1
        run_ids.append(key_to_run[key])

    return run_ids


def _build_line_scan_run_ids(blocks: List[Dict]) -> List[int]:
    """Assign run IDs for line scan result blocks (date/time based)."""
    run_ids: List[int] = []
    key_to_run: Dict[str, int] = {}
    next_run = 1
    ...
    for i, b in enumerate(blocks):
        ...
        time = _normalize_time_token(b.get('time') or '')
        if date or time:
            key = f'dt:{date}|{time}'
        else:
            # fallback key includes block order and scan geometry
            key = f'blk:{i+1}|len:{b.get("scan_length")}|count:{b.get("count")}'
...
def _deduplicate_label(label: str, seen: Set[str], type_counts: Dict[str, int] = None) -> str:
    ...
    if has_index:
        ...
        if base_label in multi_instance_types:
            ...
            dup_num = 2
            while True:
                # Keep sum naming especially simple: sum, sum_2, sum_3...
                if label == 'sum':
                    candidate = f'{label}_{dup_num}'
                else:
                    candidate = f'{label}_dup{dup_num}'
                if candidate not in seen:
                    seen.add(candidate)
...
def _find_full_deconv_spectrum_indices(
    spectra: List[Dict], deconv_element_indices: Set[int]
) -> List[int]:
    """Find generic full deconvolved spectra (energy-axis) if present.

    These are deconvolution spectra that are not mapped to per-element
    TRTDeconvolutionResult channels and do not carry spot/line qualifiers.
    """
    full_indices: List[int] = []
    for i, sp in enumerate(spectra):
        name = sp.get('name', '')
        counts = sp.get('counts', [])
        if i in deconv_element_indices:
            continue
        if _classify_spectrum(name, counts) == 'deconv':
...
class RTXParser:
    ...
    def __init__(self, filepath: str):
        ...
        self.line_scan_profiles: List[Dict] = []
        self.element_maps_2d: List[Dict] = []
    ...
    def parse(self) -> bool:
        try:
            ...
            self._extract_line_scan_profiles(self.inner_root)
            self._extract_element_maps_2d(self.inner_root)
    ...
    def _extract_line_scan_profiles(self, root):
        """Extract element intensity-vs-position line profiles from RTX data."""
        for ci in root.iter('ClassInstance'):
            if ci.get('Type') != 'TRTLineScanResults':
                continue

            block: Dict = {
                'name': ci.get('Name', 'line_scan'),
                'result_type': (ci.findtext('ResultType') or '').strip(),
                'count': int((ci.findtext('Count') or '0').strip() or 0),
                'scan_length': _parse_float(ci.findtext('ScanLength')),
                'start_x': _parse_float(ci.findtext('StartX')),
                'start_y': _parse_float(ci.findtext('StartY')),
                'end_x': _parse_float(ci.findtext('EndX')),
                'end_y': _parse_float(ci.findtext('EndY')),
                'date': (ci.findtext('Date') or '').strip(),
                'time': (ci.findtext('Time') or '').strip(),
                'series': [],
            }

            # DescriptionN fields often contain element labels in scan order.
            descriptions: Dict[int, str] = {}
            for i in range(256):
                txt = ci.findtext(f'Description{i}')
                if txt is None:
                    break
                t = txt.strip()
                if t:
            ...
            for sub in ci.findall('ClassInstance'):
                if sub.get('Type') != 'TRTBinaryData':
                    continue

                encoded = (sub.findtext('Data') or '').strip()
                if not encoded:
                    continue

                expected_size = None
                size_txt = (sub.findtext('Size') or '').strip()
                if size_txt:
                    try:
                        expected_size = int(size_txt)
                    except ValueError:
                        expected_size = None

                try:
                    values = _decode_binary_float64_series(encoded, expected_size)
                except Exception:
                    # Skip malformed series but continue processing other channels.
                    continue

                name = (sub.get('Name') or '').strip()
                if not name:
                    # fallback to Description index if unnamed
                    idx_txt = (sub.findtext('Index') or '').strip()
                    if idx_txt.isdigit():
                        name = descriptions.get(int(idx_txt), '')
                if not name:
                    name = f"series_{len(block['series']) + 1}"
            ...
            if block['series']:
                self.line_scan_profiles.append(block)

    def _extract_element_maps_2d(self, root):
        """Extract elemental 2D map planes from TRTImageData blocks."""
        valid_symbols = set(_PERIODIC_TABLE_SYMBOLS[1:])
        image_group_id = 0
        for ci in root.iter('ClassInstance'):
            if ci.get('Type') != 'TRTImageData':
                continue

            width = int(float((ci.findtext('Width') or '0').strip() or 0))
            height = int(float((ci.findtext('Height') or '0').strip() or 0))
            if width <= 0 or height <= 0:
                continue

            data_nodes = ci.findall('.//Data')
            if len(data_nodes) <= 1:
                continue
            image_group_id += 1

            xcal = _parse_float(ci.findtext('XCalibration'))
            ycal = _parse_float(ci.findtext('YCalibration'))
            plane_count = int(float((ci.findtext('PlaneCount') or '0').strip() or 0))

            descriptions = [_decode_utf16_prefixed_label((d.text or '').strip()) for d in ci.iter('Description')]
            max_planes = min(len(data_nodes), plane_count if plane_count > 0 else len(data_nodes))

            for pi in range(max_planes):
                if pi >= len(descriptions):
                    break
                desc = descriptions[pi].strip()
                if not desc:
                    continue

                m = re.match(r'^([A-Z][a-z]?)(?:\s*[-_].*)?$', desc)
                if not m:
                    continue
                ...
                encoded = (data_nodes[pi].text or '').strip()
                if not encoded:
                    continue
                try:
                ...
                self.element_maps_2d.append({
                    ...
                    'ycal': ycal,
                    'data_u16': arr,
                    'source_name': (ci.get('Name') or '').strip(),
...
# ---------------------------------------------------------------------------
# Line scan (intensity vs position) exporters
# ---------------------------------------------------------------------------

class LineScanExporter:
    """Export line-scan element profiles as CSV and MSA."""

    def __init__(self, parser: RTXParser):
        self.p = parser

    @staticmethod
    def _positions_for(n_points: int, scan_length: Optional[float]) -> List[Tuple[int, float, float]]:
        """Return tuples of (index, normalized_position, position_scan_units)."""
        if n_points <= 0:
            return []
        if n_points == 1:
            return [(0, 0.0, 0.0 if scan_length is None else scan_length)]
        positions: List[Tuple[int, float, float]] = []
        for i in range(n_points):
            frac = i / (n_points - 1)
            pos = frac * scan_length if scan_length is not None else float(i)
            positions.append((i, frac, pos))
        return positions

    @staticmethod
    def _safe_result_type(result_type: str) -> str:
        lbl = _sanitize_label(result_type or '')
        return lbl or 'unknown'

    def export_block_csv(self, block: Dict, path: str):
        """Write a multi-element profile table for one line scan block."""
        if not block.get('series'):
            raise ValueError("No line scan series in block")
        n_points = min(s.get('n_points', 0) for s in block['series'] if s.get('n_points', 0) > 0)
        if n_points <= 0:
            raise ValueError("Line scan block has empty data series")

        pos_rows = self._positions_for(n_points, block.get('scan_length'))
        headers = ['PointIndex', 'PositionFraction_0to1', 'PositionScanUnits']
        headers += [f'Intensity_{_sanitize_label(s["name"])}' for s in block['series']]

        with open(path, 'w', encoding='utf-8') as f:
            f.write(','.join(headers) + '\n')
            for i, frac, pos in pos_rows:
                row = [str(i), f'{frac:.6f}', f'{pos:.6f}']
                for s in block['series']:
                    row.append(f'{float(s["values"][i]):.6f}')
                f.write(','.join(row) + '\n')

    def export_element_msa(self, block: Dict, series: Dict, path: str, title: str):
        """Write a single element profile as an MSA Y-spectrum (x = point index)."""
        vals = series.get('values', [])
        if not vals:
            raise ValueError("Empty line profile series")
        n = len(vals)
        CRLF = '\r\n'
        lines: List[str] = []
        ...
        kv('NPOINTS', n)
        kv('NCOLUMNS', 1)
        kv('XUNITS', 'point_index')
        kv('YUNITS', 'intensity')
        kv('DATATYPE', 'Y')
        kv('XPERCHAN', '1.000000')
        kv('OFFSET', '0.000000')
        kv('SIGNALTYPE', 'EDS_LINE_PROFILE')
        if block.get('scan_length') is not None:
            kv('COMMENT', f"ScanLength={block['scan_length']}")
        if block.get('result_type'):
            kv('COMMENT', f"ResultType={block['result_type']}")
        kv('SPECTRUM', 'Line profile intensity data starts here')
        for v in vals:
            vv = 0.0 if (isinstance(v, float) and (math.isnan(v) or math.isinf(v))) else float(v)
            lines.append(f'{vv:.6f}, ')
        kv('ENDOFDATA', 'End Of Data and File')

        with open(path, 'w', encoding='latin-1', newline='') as f:
            f.write(CRLF.join(lines))

    def export_element_csv(self, block: Dict, series: Dict, path: str):
        """Write one element line profile as CSV (position vs intensity)."""
        vals = series.get('values', [])
        if not vals:
            raise ValueError("Empty line profile series")
        pos_rows = self._positions_for(len(vals), block.get('scan_length'))
        with open(path, 'w', encoding='utf-8') as f:
            f.write('PointIndex,PositionFraction_0to1,PositionScanUnits,Intensity\n')
            for (i, frac, pos), v in zip(pos_rows, vals):
                f.write(f'{i},{frac:.6f},{pos:.6f},{float(v):.6f}\n')


# ---------------------------------------------------------------------------
# Elemental 2D map exporter
# ---------------------------------------------------------------------------

class ElementMapMSAExporter:
    """Export elemental 2D maps as flattened MSA (HyperSpy-readable)."""
    ...
    def _latin1_safe(text: str) -> str:
        """Make comment text safe for latin-1 MSA output."""
        return (text or '').encode('latin-1', errors='ignore').decode('latin-1')

    def export_flattened_msa(self, map_item: Dict, path: str, title: str):
        data = map_item.get('data_u16')
        if not data:
        ...
        kv('NPOINTS', n)
        kv('NCOLUMNS', 1)
        kv('XUNITS', 'pixel_index')
        kv('YUNITS', 'intensity')
        kv('DATATYPE', 'Y')
        kv('XPERCHAN', '1.000000')
        kv('OFFSET', '0.000000')
        kv('SIGNALTYPE', 'EDS_ELEMENT_MAP_2D_FLATTENED')
        kv('COMMENT', f"Element={self._latin1_safe(map_item.get('description', ''))}")
        kv('COMMENT', f"MapWidth={map_item.get('width')}")
        kv('COMMENT', f"MapHeight={map_item.get('height')}")
        if map_item.get('xcal') is not None:
            kv('COMMENT', f"XCalibration_um_per_px={map_item['xcal']}")
        if map_item.get('ycal') is not None:
            kv('COMMENT', f"YCalibration_um_per_px={map_item['ycal']}")
        kv('COMMENT', "FlattenOrder=row_major")
        kv('COMMENT', "ReshapeHint=Use (MapHeight,MapWidth) in HyperSpy/Python")
        kv('SPECTRUM', 'Flattened 2D elemental map intensity data starts here')
        for v in data:
            lines.append(f'{float(v):.6f}, ')
        kv('ENDOFDATA', 'End Of Data and File')
...
def convert_rtx_file(rtx_path: str, output_dir: str = None) -> bool:
    ...
    if not parser.parse():
        return False
    sample_token = _build_sample_token(parser.metadata, stem)
    ...
    deconv_element_indices = set(parser.deconv_element_symbols_by_spectrum_idx.keys())
    full_deconv_indices = _find_full_deconv_spectrum_indices(parser.spectra, deconv_element_indices)
    spectrum_run_ids = _build_spectrum_run_ids(parser.spectra, parser.metadata)
    ...
    for i, sp in enumerate(parser.spectra):
        ...
        run_id = spectrum_run_ids[i] if i < len(spectrum_run_ids) else 1
        if run_id > 1:
            label = f'{label}_run{run_id}'
        label = _deduplicate_label(label, seen, type_counts)
        labels.append(label)

    full_deconv_serial = 0
    for i, sp in enumerate(parser.spectra):
        # User-requested behavior: do not export energy-axis single-element
        # deconvolution spectra. Spatial (intensity-by-position) outputs are
        # exported separately from TRTLineScanResults below.
        if labels[i].startswith('deconv_') or labels[i].startswith('empty_deconv_'):
            if i in full_deconv_indices:
                full_deconv_serial += 1
                suffix = '_deconv_full_spectrum'
                if full_deconv_serial > 1:
                    suffix = f'{suffix}_{full_deconv_serial}'
                msa_path = out / f'{stem}{suffix}.msa'
                try:
                    EMSAExporter(parser).export(str(msa_path), i, title='deconv_full_spectrum')
                    print(f"  Created  : {msa_path.name}  ({len(sp['counts'])} channels, full deconvolved EMSA/MSA 1.0)")
                except Exception as e:
                    print(f"  WARNING  : full deconv EMSA export failed – {e}")
            continue
    ...
    # Line scan spatial profiles (element intensity vs position)
    if parser.line_scan_profiles:
        lsx = LineScanExporter(parser)
        ls_seen: Set[str] = set()
        ls_type_counts: Dict[str, int] = {}
        ls_run_ids = _build_line_scan_run_ids(parser.line_scan_profiles)
        for bi, block in enumerate(parser.line_scan_profiles, start=1):
            block_run_id = ls_run_ids[bi - 1] if bi - 1 < len(ls_run_ids) else 1
            rt_lbl = LineScanExporter._safe_result_type(block.get('result_type', ''))

            # Combined CSV for all elements in this line scan result block
            csv_name = (
                f"{stem}_line_scan_run_{block_run_id}_block_{bi:02d}_result_{rt_lbl}"
                f"_elements_intensity_by_position.csv"
            )
            csv_path = out / csv_name
            try:
                lsx.export_block_csv(block, str(csv_path))
                print(f"  Created  : {csv_path.name}")
            except Exception as e:
                print(f"  WARNING  : line scan CSV export failed – {e}")

            # One MSA per element series in this block
            for series in block.get('series', []):
                elem = _sanitize_label(series.get('name', 'element')) or 'element'
                base = f'deconv_line_{elem}'
                if block_run_id > 1:
                    base = f'{base}_run{block_run_id}'
                base_label = _deduplicate_label(base, ls_seen, ls_type_counts)
                msa_name = f"{stem}_{base_label}.msa"
                elem_csv_name = f"{stem}_{base_label}.csv"
                msa_path = out / msa_name
                elem_csv_path = out / elem_csv_name
                title = f"line_scan_block_{bi:02d}_{rt_lbl}_{elem}_intensity_vs_position"
                try:
                    lsx.export_element_msa(block, series, str(msa_path), title=title)
                    print(f"  Created  : {msa_path.name}  ({series.get('n_points', 0)} points, line profile MSA)")
                except Exception as e:
                    print(f"  WARNING  : line scan MSA export failed – {e}")
                try:
                    lsx.export_element_csv(block, series, str(elem_csv_path))
                    print(f"  Created  : {elem_csv_path.name}")
                except Exception as e:
                    print(f"  WARNING  : line scan element CSV export failed – {e}")

    # Elemental 2D maps (flattened MSA for HyperSpy/ExSpy workflows)
    if parser.element_maps_2d:
        mexp = ElementMapMSAExporter(parser)
        map_seen: Set[str] = set()
        map_counts: Dict[str, int] = {}
        for m in parser.element_maps_2d:
            group_id = int(m.get('image_group_id', 0) or 0)
            group_token = f'img{group_id:02d}' if group_id > 0 else 'img00'
            base = _deduplicate_label(f"deconv_map_{m['symbol']}_{group_token}", map_seen, map_counts)
            msa_name = f"{stem}_sample_{sample_token}_{base}_intensity_2d_flattened.msa"
            msa_path = out / msa_name
            title = f"{base}_2d_map_flattened_for_hyperspy"
            try:
                mexp.export_flattened_msa(m, str(msa_path), title=title)
                print(f"  Created  : {msa_path.name}  ({m.get('width')}x{m.get('height')} map, flattened MSA)")
            except Exception as e:
                print(f"  WARNING  : elemental map MSA export failed – {e}")
